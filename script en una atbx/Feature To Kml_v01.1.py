...
numero_registros_int = int(numero_registros.getOutput(0))
""" ----------------- """
...
if os.path.exists(carpeta_resultado):
    ...
    arcpy.AddMessage("\nSe ha creado la carpeta " + nombre_carpeta_resultado)
# ---------------------------------------------------------------------------
# Comprobando que no hay en el TOC ninguna capa temporal de esta herramienta
...
with arcpy.da.SearchCursor(capa_entrada, ['OID@', campo_nombre]) as cursor:
    ...
    for row in cursor:
        ...
        fraccion = round((i / numero_registros_int) * 100, 1)
        if fraccion_aux == 0 or fraccion > fraccion_aux + salto_mensaje_porcentajes or fraccion >= 100:
